...
RANDOM_HISTORY_FILE = os.path.join(SCRIPT_DIR, "puzzle71_random_done.txt")
...
def search_key_chunked(args):
    ...
    worker_id, RANGE_START, RANGE_END, target_address = args
    local_count = 0
        ...
        if search_mode == "random":
            with Manager() as manager:
                ...
                with Pool(
                    ...
                    wait_for_pool(p, search_method, [(i, TARGET_BTC_ADDRESS) for i in range(num_workers)])
        else:
            with Pool(
                num_workers,
                ...
                wait_for_pool(p, search_method, [(i, RANGE_START, RANGE_END, TARGET_BTC_ADDRESS) for i in range(num_workers)])
    except KeyboardInterrupt:
